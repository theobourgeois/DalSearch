...
export async function getAcademicCalendar() {
    ...
    let subjectCodes = fs.readFileSync("../../utils/subjects.json", "utf8");
    subjectCodes = JSON.parse(subjectCodes);
    let i = 1;
    ...
    for (const code of subjectCodes) {
        ...
        try {
            ...
            console.log(
                `------ [${i}/${subjectCodes.length}: FETCHING COURSES WITH SUBJECT: ${code.description}] ------`
            );
...
async function main() {
    ...
    let previousCourses = fs.readFileSync(outputFilename, "utf8");
...
async function fillInMissingData(courses) {
        ...
        const promise = async () => {
            try {
                ...
                console.log(
                    `------ [${i + 1}/${
                        codes.length
                    }: FETCHING DESCRIPTION FOR COURSE: ${code}] ------`
                );
                i++;
    ...
    const waitTimeSeconds = codes.length * (0.5 + 0.1);
    console.log(
        "Estimated wait time: ",
